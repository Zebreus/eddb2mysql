...
if [ ! -f ${DATADIR}/factions.csv ]; then
	wget -O - -S --header="accept-encoding: gzip" https://eddb.io/archive/v6/factions.csv | gzip -dc | tail -n +2 > ${DATADIR}/factions.csv
fi
if [ ! -f ${DATADIR}/listings.csv ]; then
	wget -O - -S --header="accept-encoding: gzip" https://eddb.io/archive/v6/listings.csv | gzip -dc | tail -n +2 > ${DATADIR}/listings.csv
fi
if [ ! -f ${DATADIR}/systems_import.csv ]; then
	wget -O - -S --header="accept-encoding: gzip" https://eddb.io/archive/v6/systems.csv | gzip -dc | tail -n +2 | split -d -l 10000000 - ${DATADIR}/systems_import.csv
...
if [ ! -f ${DATADIR}/modules.csv ]; then
	wget -O - -S --header="accept-encoding: gzip" https://eddb.io/archive/v6/modules.json | gzip -dc | json2csv/bin/json2csv.js -f "id","group_id","class","rating","price","weapon_mode","missile_type","name","belongs_to","ed_id","ed_symbol","ship" | tail -n +2 > ${DATADIR}/modules.csv
fi
...
if [ ! -f ${DATADIR}/stations_import.csv ]; then
	wget -O - -S --header="accept-encoding: gzip" https://eddb.io/archive/v6/stations.json | gzip -dc | json2csv/bin/json2csv.js -f "id","name","system_id","updated_at","max_landing_pad_size","distance_to_star","government_id","allegiance_id","type_id","type","has_blackmarket","has_market","has_refuel","has_repair","has_rearm","has_outfitting","has_shipyard","has_docking","has_commodities","shipyard_updated_at","outfitting_updated_at","market_updated_at","is_planetary","settlement_size_id","settlement_size","settlement_security_id","settlement_security","body_id","controlling_minor_faction_id" | tail -n +2 > ${DATADIR}/stations_import.csv
fi
